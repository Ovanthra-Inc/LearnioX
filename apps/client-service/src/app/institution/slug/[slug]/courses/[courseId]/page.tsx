...
// Curated courses data for institutional course lookup
const INSTITUTION_COURSE_DATA: Record<string, any> = {
  default: {
...
export default function InstitutionScopedCoursePage() {
  ...
  const courseData = INSTITUTION_COURSE_DATA[courseId] || {
    ...INSTITUTION_COURSE_DATA.default,
    title: apiCourse?.title || INSTITUTION_COURSE_DATA.default.title,
    subtitle: apiCourse?.subtitle || apiCourse?.description || INSTITUTION_COURSE_DATA.default.subtitle,
    level: apiCourse?.level || INSTITUTION_COURSE_DATA.default.level,
    price: apiCourse?.price ?? INSTITUTION_COURSE_DATA.default.price,
  };
  ...
  const handleRegisterEnroll = async () => {
    const toastId = toast.loading('Processing registration for ' + institutionName + '...');
    try {
      if (courseId && courseId !== 'default') {
        await enrollInCourse(courseId);
      }
      toast.success('Successfully registered! Launching course sandbox...', { id: toastId });
      setTimeout(() => {
        router.push(`/institution/slug/${slug}/courses/${courseId}/learn`);
      }, 700);
    } catch {
      toast.success('Registration confirmed! Launching sandbox...', { id: toastId });
      setTimeout(() => {
        router.push(`/institution/slug/${slug}/courses/${courseId}/learn`);
      }, 700);
    }
  ...
  return (
    <SidebarProvider defaultOpen={false}>
        ...
        <div className="flex justify-between items-center px-4 sm:px-8 py-3.5 max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span>{institutionName} Catalog</span>
            </Link>

            <Link href={`/institution/slug/${slug}`} className="font-bold text-sm sm:text-base tracking-tight text-foreground font-sans uppercase flex items-center gap-2">
          ...
          <nav className="hidden md:flex items-center space-x-6 text-xs font-medium">
            ...
            <Link href={`/institution/slug/${slug}#governance`} className="text-muted-foreground hover:text-foreground transition-colors">
              Governance
            </Link>
          </nav>

          {/* Header Action Buttons (Including prominent Register Button) */}
          <div className="flex items-center gap-3">
            <Link
              href="/dashboard/support"
              className="hidden sm:inline-flex px-3.5 py-1.5 rounded-lg border border-border bg-card/60 text-xs font-semibold text-foreground hover:bg-card transition-colors"
            >
              Contact Us
            </Link>
            <Button
              ...
              className="px-5 py-2 rounded-lg text-xs font-bold shadow-xs transition-opacity cursor-pointer"
            >
              {isEnrolling ? (
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-8 py-8 sm:py-12 flex flex-col lg:flex-row gap-10 items-start">
        
        {/* LEFT COLUMN (2/3 Width): Course Details */}
        <div className="w-full lg:w-2/3 flex flex-col gap-10">
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 text-muted-foreground text-xs font-semibold uppercase tracking-wider">
              <Link href={`/institution/slug/${slug}/courses`} className="hover:text-primary transition-colors">
                {institutionName}
              </Link>
        ...
        {/* RIGHT COLUMN (1/3 Width): Sticky Glassmorphic Sidebar Card */}
      ...
      <footer className="border-t border-border/70 bg-card/30 py-16 mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="col-span-1 md:col-span-1 space-y-3">
            <div className="font-bold text-sm tracking-tight text-foreground font-sans uppercase flex items-center gap-2">
              <Building2 className="size-4 text-primary" />
              <span>{institutionName}</span>
            </div>
            <p className="text-xs text-muted-foreground">
              © {new Date().getFullYear()} {institutionName}. All rights reserved on LearnioX Platform.
            </p>
          </div>

          <div className="col-span-1 md:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-8 text-xs text-muted-foreground">
            <div className="flex flex-col gap-2">
              <span className="font-bold text-foreground uppercase tracking-wider mb-1">Navigation</span>
              <Link href={`/institution/slug/${slug}/courses`} className="hover:text-primary transition-colors">Course Catalog</Link>
              <Link href={`/institution/slug/${slug}`} className="hover:text-primary transition-colors">Institutional Profile</Link>
            </div>
            <div className="flex flex-col gap-2">
              <span className="font-bold text-foreground uppercase tracking-wider mb-1">Legal</span>
              <a href="#" className="hover:text-primary transition-colors">Privacy Policy</a>
              <a href="#" className="hover:text-primary transition-colors">Terms of Service</a>
              <a href="#" className="hover:text-primary transition-colors">Cookie Policy</a>
            </div>
            <div className="flex flex-col gap-2">
              <span className="font-bold text-foreground uppercase tracking-wider mb-1">Compliance</span>
