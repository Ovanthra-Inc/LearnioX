  ...
  (
      ...
      return (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div
            ref={ref}
            className={cn(
              "relative z-50 flex h-full w-[--sidebar-width] flex-col border-r border-sidebar-border bg-sidebar p-2 text-sidebar-foreground shadow-xl animate-in slide-in-from-left",
              className
            )}
            {...props}
          >
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "group peer hidden md:block shrink-0 text-sidebar-foreground transition-[width] duration-200 ease-linear",
          isIcon ? "w-12" : "w-64"
        )}
        data-state={state}
      ...
      >
        <div
          className={cn(
            "duration-200 fixed inset-y-0 z-20 hidden h-svh transition-[left,right,width] ease-linear md:flex flex-col border-r border-sidebar-border bg-sidebar",
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "inline-flex h-7 w-7 items-center justify-center rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground transition-colors cursor-pointer",
        className
      )}
      onClick={(event) => {
        onClick?.(event)
...
>(({ asChild = false, size = "md", isActive, className, children, ...props }, ref) => {
  if (asChild && React.isValidElement(children)) {
    return React.cloneElement(children as React.ReactElement<any>, {
      className: cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none hover:bg-sidebar-accent hover:text-sidebar-accent-foreground text-xs",
